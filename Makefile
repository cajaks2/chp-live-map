...
help:
	@printf '%s\n' \
		...
		'  make build VERSION=0.1.85         Build and push linux/amd64 image' \
		'  make update-manifest VERSION=0.1.85 Update image tags and SERVICE_VERSION in k8s manifest' \
		'  make apply                        kubectl apply manifest' \
		'  make rollout                      Wait for web deployment rollout' \
		'  make deploy VERSION=0.1.85        Build, push, update manifest, apply, and wait' \
